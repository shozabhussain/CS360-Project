...
const AuthPopBox = (props) => {
	...
	const registerWithEmail = (signinEmail, signinPassword) => {
		// // Admin Account
		if (signinEmail === "admin@admin.com" && signinPassword === "admin") {
			router.push("/admin-dashboard/pending-proposals");
			return;
		}
				...
				<div className="relative w-auto my-6 mx-auto max-w-3xl">
					...
					<div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full custom-bg-offwhite outline-none focus:outline-none">
							...
							<div>
								<ul className="flex mb-0 list-none flex-wrap pt-3 pb-4 flex-row" role="tablist">
									<li className="-mb-px last:mr-0 flex-auto text-right">
										<a
											className={"text-xs pl-2 font-bold uppercase px-5 py-3 shadow-lg rounded block leading-normal " + (openTab === 1 ? "custom-bg-yellow " : "text-blueGray-600 bg-white")}
											onClick={(e) => {
												e.preventDefault();
												setOpenTab(1);
											}}
									...
									<li className="-mb-px last:mr-0 flex-auto text-left">
										<a
											className={"text-xs font-bold uppercase px-5 py-3 shadow-lg rounded block leading-normal " + (openTab === 2 ? "custom-bg-yellow" : "text-blueGray-600 bg-white")}
											onClick={(e) => {
												e.preventDefault();
												setOpenTab(2);
											}}
									...
									<li className="-mb-px mr-2 last:mr-0 text-center">
										<button className="p-1 ml-auto bg-transparent border-0 text-black float-right text-3xl leading-none font-semibold outline-none focus:outline-none" onClick={() => props.showLoginProp(false)}>
						...
						<div className="relative p-6 flex-auto">
							<div className={openTab === 1 ? "block" : "hidden"} id="link1">
								{/* Login Form */}
								<p className="custom-txt-title text-black">Enter your email and password below:</p>
								<p className="custom-txt-normal text-black">To continue logging into the account</p>
								{loginerrorShow ? (
									<div className=" bg-red-600 text-white p-2 mt-2 mb-2 rounded-lg flex w-full justify-center items-center">
										<i className="fa fa-exclamation-circle mr-2"></i> {loginerror}
									</div>
								) : null}
								<form
									onSubmit={(e) => {
								...
								>
									...
									<a className="text-center text-gray-400 custom-txt-sml" href="" onClick={() => passwordResetEmail(signinEmail)}>
										Reset Password?
									</a>
								</form>
							</div>
							<div className={openTab === 2 ? "block" : "hidden"} id="link2">
								{/* Sign Up Form */}
								<p className="custom-txt-title text-black">Enter your email and password below:</p>
								<p className="custom-txt-normal text-black">OWN . Transfer . Verify</p>
								{/* {signUperrorShow ? <div className="bg-red-400 text-white p-2">Error! {signUperror}</div> : null} */}
								{signUperrorShow ? (
									<div className=" bg-red-600 text-white p-2 mt-2 mb-2 rounded-lg flex w-full justify-center items-center">
										<i className="fa fa-exclamation-circle mr-2"></i> {signUperror}
									</div>
								) : null}
								<form
									onSubmit={(e) => {
                  ...
                  /> */}
									...
									<div className="relative w-full mb-3">
										...
										/>
										{signupPasswordFirst != signupPasswordSec || signupPasswordFirst == "" ? (
											<p className="custom-txt-sml text-red-600">
												Password <span className="font-bold underline">NOT</span> Match!
											</p>
										) : (
											<p className="custom-txt-sml text-red-600">
												Password <span className="font-bold underline">Matched!</span>
											</p>
										)}
									</div>
						...
						{openTab == 1 ? (
							<div className="p-6 flex-auto -mt-10">
								<div className="custom-txt-sml text-center text-black">
									...
									<a href="" onClick={() => setOpenTab(2)} className="custom-a-blue cursor-pointer">
										Sign Up NOW!.
									</a>
								</div>
