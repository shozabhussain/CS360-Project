...
export default function Index() {
	...
	const getManufacturers = () => {
		const q = query(collection(db, "verification-proposals"), where("status", "==", "pending"));
	...
	const handleAccept = (index) => {
		const docId = manufacturersList[index].docId;

		const docRef = doc(db, "verification-proposals", docId);

		updateDoc(docRef, {
			status: "approved",
		}).then(() => {
			Swal.fire("Proposal Accepted!");
			setTimeout(() => {
				setRender(!render);
			}, 100);
		});
								...
								<div>
									...
									<div className="flex flex-wrap  bg-white">
										<div className="ml-auto  pt-6 flex">
											<button onClick={() => handleReject(i)} className="custom-bg-red flex ml-auto hover:bg-blue-700 mb-4 text-white w-2/12 font-bold  px-12 rounded justify-center">
												Decline
											</button>

											<button onClick={() => handleAccept(i)} className="custom-bg-red flex mr-8 ml-auto  hover:bg-blue-700 mb-4 text-white w-2/12 font-bold px-12 rounded justify-center">
												Accept
											</button>
										</div>
									</div>
